...
public class H2UserDao implements UserDaoInterface
{
    ...
    public User createUser(String name, String surname, String email, String password, UserType type, String login) {
        try (
                Connection connection = dataSource.getConnection();
                PreparedStatement statement = connection.prepareStatement("INSERT INTO USERS (NAME, SURNAME, LOGIN, EMAIL, PASSWORD, TYPE)" +
            ...
            )
        {
            ...
            try (ResultSet geteratedKeys = statement.getGeneratedKeys())
            {
                if (geteratedKeys.next())
                {
                    ...
                    return new User(id, name, surname, email, password, login, type);
                }
                else
                {
